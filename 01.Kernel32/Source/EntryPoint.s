...
; 코드 영역
START:
	mov ax, 0x1000	; 보호 모드 엔트리 포인트의 시작 어드레스(0x1000)를 세그먼트 레지스터 값으로 변환
	...
	; BIOS 서비스를 사용해서 A20게이트를 활성화
	mov ax, 0x2401	; A20게이트 활성화 서비스 설정
	int 0x15		; BIOS 인터럽트 서비스 호출

	jc .A20GATEERROR	; A20게이트 활성화가 성공했는지 확인
	jmp .A20GATESUCCESS
...
.A20GATEERROR:
	...
	and al, 0xFE	; 시스템 리셋 방지를 위해 0xFE와 AND 연산하여 비트 0 를 0으로 설정
...
.A20GATESUCCESS:
	...
	mov eax, 0x4000003B	; PG=0, CD=1, AM=0, WP=0, NE=1, ET=1, TS=1, EM=0, MP=1, PE=1
...
PROTECTEDMODE:
	...
	jmp dword 0x18: 0x10200		; CS 세그먼트 셀렉터를 커널 코드 디스크립터(0x08)로 변경하면서
...
.MESSAGELOOP:			; 메세지 출력 루프
	...
	je .MESSAGEEND		; 복사한 문자의 값이 0이면 문자열이 종료되었으므로 .MESSAGEEND로 이동하여 문자 출력 종료
