...
; CPUID를 반환
; PARAM: DWORD dwEAX, DWORD* pdwEAX, * pdwEBX, * pbwECX, * pdwEDX
kReadCPUID:
	push ebp		; 베이스 포인터 레지스터를 스텍에 삽입
	mov ebp, esp	; 베이스 포인터 레지스터에 스택포인터 레지스터의 값을 설정
...
kSwitchAndExecute64bitKernel:
	...
	mov eax, cr4	; CR4 컨트롤러 레지스터의 값을 EAX레지스터에 저장
	...
	or eax, 0x0100		; EAX레지스터에 저장된 IA32)EFER MSR의 하위 32비트에서 LME 비트(비트8)을 1로 설정
	wrmsr				; MSR 레지스터에 쓰기

	; CR0 컨트롤 레지스터를 NW 비트(비트 29) = 0, CD 비트(비트 30) = 0, PG 비트(비트 31) = 1로
    ; 설정하여 캐시 기능과 페이징 기능을 활성화하고 TS 비트(비트 3) = 1, EM 비트(비트 2) = 0, MP 비트(비트 1) = 1로 설정하여 FPU를 활성화
	mov eax, cr0		; EAX 레지스터에 CR0 컨트롤 레지스터를 저장
	or eax, 0xE000000E	; NW 비트(비트 29), CD 비트(비트 30), PG 비트(비트 31),
						; TS 비트(비트 3), EM 비트(비트 2), MP 비트(비트 1)을 모두 1로 설정
	xor eax, 0x60000004	; NW 비트(비트 29), CD 비트(비트 30), EM 비트(비트 2), EM 비트(비트 2)를 XOR하여 0으로 설정
