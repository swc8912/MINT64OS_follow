...
// 램의 총 크기(MB 단위)
static gs_qwTotalRAMSize = 0;

// 64MB 이상의 위치부터 램 크기를 체크 최초 부팅 과정에서 한 번만 호출해야 함
void kCheckTotalRAMSize(void)
{
	DWORD* pdwCurrentAddress;
	DWORD dwPreviousValue;

	// 64MB(0x400000)부터 4MB 단위로 검사 시작
...
// vsprintf 함수의 내부 구현  버퍼에 포맷 문자열에 따라 데이터를 복사
int kVSPrintf(char* pcBuffer, const char* pcFormatString, va_list ap)
{
	...
	// NULL을 추가해 완전한 문자열로 만들고 출력할 문자의  길이를 반환
