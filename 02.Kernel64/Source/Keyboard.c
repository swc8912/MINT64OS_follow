...
{
	// 상태 레지스터(포트 0x64)에서 읽은 값에 입력 버퍼 상태 비트(비트 1)가 1로 설정되어 있으면 아직 키보드가 데이터를 가져가지 않음
...
BOOL kWaitForACKAndPutOtherScanCode(void)
{
	...
	// ACK가 오기 전에 키보드 출력 버퍼(포트 0x60)에 키 데이터가 저장될 수 있으므로 키보드에서 전달된 데이터를 최대 100개까지 수신하여 ACK 확인
	for(j = 0; j < 100; j++)
	{
		// 0xFFFF만큼 루프를 수행할 시간이면 충분히 커맨드의 응답이 올 수 있음
		// 0xFFFF루프를 수행한 이후에도 출력 버퍼(포트 0x60)가 차 있지 않으면 무시하고 읽음
		for(i = 0; i < 0xFFFF; i++)
		{
		...
		// ACK가 아니면 ASCII 코드로 변환하여 키 큐에 삽입
		else
		{
...
BOOL kActivateKeyboard(void)
{
	...
	for(i = 0; i < 0xFFFF; i++)
	{
		// 입력 버퍼(포트 0x60)가 비어있으면 키보드 커맨드 전송
...
void kEnableA20Gate(void)
{
	...
	kOutPortByte(0x64, bOutputPortData);
}
...
{
	// 변환 테이블 값을 직접 읽어서 알파벳 범위(스캔 코드 2~53)에서 영문자가 아니면 숫자나 기호임
...
void UpdateCombinationKeyStatusAndLED(BYTE bScanCode)
{
	...
	// NumLock 키의 스캔 코드(69)이면 NumLock의 상태 갱신하고 LED 상태 변경
	else if((bDownScanCode == 69) && (bDown == TRUE))
	{
		gs_stKeyboardManager.bNumLockOn ^= TRUE;
		bLEDStatusChanged = TRUE;
	}
	// ScrollLock 키의 스캔 코드(70)이면 ScrollLock의 상태 갱신하고 LED 상태 변경
