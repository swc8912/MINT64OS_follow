...
// 하드 디스크의 MBR을 읽어서 MINT 파일 시스템인지 확인 MINT 파일 시스템이라면 파일 시스템에 관련된 각종 정보를 읽어서 자료구조에 삽입
BOOL kMount(void)
{
	MBR* pstMBR;
...
BOOL kFormat(void)
{
	...
	// 링크 데이터는 4바이트이므로, 한 섹터에는 128개가 들어감. 따라서 총 개수를 128로 나눈 후 올림하여 클러스터 링크의 섹터 수를 구함
	dwClusterLinkSectorCount = (dwMaxClusterCount + 127) / 128;

	// 예약된 영역은 현재 사용하지 않으므로, 디스크 전체 영역에서 MBR 영역과 클러스터 링크 테이블 영역의 크기를 뺀 나머지가 실제 데이터 영역이 됨
...
{
	// 데이터 영역의 시작 어드레스를 더함
	return gs_pfReadHDDSector(TRUE, TRUE, (dwOffset * FILESYSTEM_SECTORSPERCLUSTER)	+ gs_stFileSystemManager.dwDataAreaStartAddress,
...
{
	// 데이터 영역의 시작 어드레스를 더함
	return gs_pfWriteHDDSector(TRUE, TRUE, (dwOffset * FILESYSTEM_SECTORSPERCLUSTER) + gs_stFileSystemManager.dwDataAreaStartAddress,
			FILESYSTEM_SECTORSPERCLUSTER, pbBuffer);
}
...
static DWORD kFindFreeCluster(void)
{
	...
	{
		// 클러스터 링크 테이블의 마지막 섹터이면 전체 섹터만큼 도는 것이 아니라 남은 클러스터의 수만큼 루프를 돌아야 함
...
static BOOL kFreeClusterUntilEnd(DWORD dwClusterIndex)
{
	...
	{
		// 다음 클러스터의 인덱스를 가져옴
		if(kGetClusterLinkData(dwCurrentClusterIndex,
		...
		if(kSetClusterLinkData(dwCurrentClusterIndex,
				FILESYSTEM_FREECLUSTER) == FALSE)
		{
			return FALSE;
		}
...
DWORD kWriteFile(const void* pvBuffer, DWORD dwSize, DWORD dwCount, FILE* pstFile)
{
	...
	while(dwWriteCount != dwTotalCount)
	{
		...
		if(pstFileHandle->dwCurrentClusterIndex == FILESYSTEM_LASTCLUSTER)
		{
			...
			if(kSetClusterLinkData(dwAllocatedClusterIndex,
...
int kSeekFile(FILE* pstFile, int iOffset, int iOrigin)
{
	...
	// 옵션에 따라서 실제 위치를 계산 음수이면 파일의 시작 방향으로 이동하며 양수이면 파일의 끝 방향으로 이동
	switch(iOrigin)
	{
	// 파일의 시작을 기준으로 이동
	...
	case FILESYSTEM_SEEK_END:
		// 이동할 오프셋이 음수이고 현재 파일 포인터의 값보다 크다면 더 이상 갈 수 없으므로 파일의 처음으로 이동
	...
	// 파일을 구성하는 클러스터의 개수와 현재 파일 포인터의 위치를 고려하여 옮겨질 파일 포인터가 위치하는 클러스터까지 클러스터 링크를 탐색
