...
static void kWriteSector(const char* pcParameterBuffer)
{
	...
	// 쓰기 수행
	if
	(kWriteHDDSector( TRUE, TRUE, dwLBA, iSectorCount, pcBuffer) != iSectorCount)
	{
		kPrintf("Write Fail\n");
		return;
	}
...
static void kReadDataFromFile(const char* pcParameterBuffer)
{
	...
	while(1)
	{
		...
		// 만약 엔터 키이면 엔터 키 횟수를 증가시키고 20라인까지 출력했다면 더 출력할지 여부를 물어봄
		if(bKey == KEY_ENTER)
		{
			iEnterCount++;
...
static void kTestFileIO(const char* pcParameterBuffer)
{
	...
	// 열린 핸들을 가지고 읽기 수행 후 데이터 검증. Random Write로 데이터가 잘못 저장될 수 있으므로 검증은 4Mbyte 전체를 대상으로 함
