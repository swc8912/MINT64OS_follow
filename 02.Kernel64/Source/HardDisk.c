...
int kReadHDDSector(BOOL bPrimary, BOOL bMaster, DWORD dwLBA, int iSectorCount, char* pcBuffer)
{
	...
	if((gs_stHDDManager.bHDDDetected == FALSE) || (iSectorCount <= 0)
			|| (256 < iSectorCount)	|| ((dwLBA + iSectorCount) >= gs_stHDDManager.stHDDInformation.dwTotalSectors))
	{
		return 0;
	}
	...
	// 데이터 레지스터 설정
	// LBA 모드의 경우, 섹터 번호 -> 실린더 번호 -> 헤드 번호의 순으로 LBA 어드레스를 대입
	...
	// 드라이브/헤드 레지스터(포트 0x1F6 또는 0x176)에 읽을 섹터의 위치(LBA 24~27비트)와 설정된 값을 같이 전송
...
// 하드 디스크에 섹터를 씀 최대 256개의 섹터를 쓸 수 있음 실제로 쓴 섹터 수를 반환
int kWriteHDDSector(BOOL bPrimary, BOOL bMaster, DWORD dwLBA, int iSectorCount,	char* pcBuffer)
{
	...
	// 범위 검사. 최대 256 섹터를 처리할 수 있음
	if ((gs_stHDDManager.bCanWrite == FALSE) || (iSectorCount <= 0)
			|| (256 < iSectorCount)	|| ((dwLBA + iSectorCount) >= gs_stHDDManager.stHDDInformation.dwTotalSectors))
	{
		return 0;
	}
	...
	// 데이터 레지스터 설정 LBA 모드의 경우, 섹터 번호 -> 실린더 번호 -> 헤드 번호의 순으로 LBA 어드레스를 대입
	...
	// 드라이브/헤드 레지스터(포트 0x1F6 또는 0x176)에 쓸 섹터의 위치(LBA 24~27비트)와 설정된 값을 같이 전송
	kOutPortByte(wPortBase + HDD_PORT_INDEX_DRIVEANDHEAD, bDriveFlag | ((dwLBA >> 24) & 0x0F));
