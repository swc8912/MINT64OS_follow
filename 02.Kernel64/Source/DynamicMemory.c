...
void kInitializeDynamicMemory(void)
{
	...
	gs_stDynamicMemory.pstBitmapOfLevel = (BITMAP*)(DYNAMICMEMORY_START_ADDRESS	+ (sizeof(BYTE)
...
static QWORD kCalculateDynamicMemorySize(void)
{
	...
	// 3GB 이상의 메모리에는 비디오 메모리와 프로세서가 사용하는 레지스터가 존재하므로 최대 3GB까지만 사용
...
// 동적 메모리 영역을 관리하는데 필요한 정보가 차지하는 공간을 계산 최소 블록 단위로 정렬해서 반환
...
// 버디 블록 알고리즘으로 메모리 블록을 할당 메모리 크기는 버디 블록의 크기로 요청해야 함
...
static int kFindFreeBlockInBitmap(int iBlockListIndex)
{
	...
	for(i = 0; i < iMaxCount;)
	{
		// QWORD는 8 * 8비트 => 64비트이므로, 64비트를 한꺼번에 검사해서 1인 비트가 있는 지 확인함
		if (((iMaxCount - i) / 64) > 0)
		{
