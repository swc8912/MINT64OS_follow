...
// 스케줄러 관련
// 스케줄러를 초기화, 스케줄러를 초기화하는데 필요한 TCB 풀과 init 태스크도 같이 초기화
...
// 인터럽트가 발생했을 때, 다른 태스크를 찾아 전환 반드시 인터럽트나 예외가 발생했을 때 호출해야 함
			...
			while(1)
			{
				...
				if(pstTask->qwFlags & TASK_FLAGS_PROCESS)
				{
				    ...
				    for(i = 0; i < iCount; i++)
				    {
				        ...
				        // 자식 스레드 리스트에 연결된 정보는 태스크 자료구조에 있는 stThreadLink의 시작 어드레스이므로, 태스크 자료구조의 시작
