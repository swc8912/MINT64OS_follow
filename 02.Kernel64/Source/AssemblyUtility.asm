...
; PARAM: 포트 번호
kInPortByte:
	push rdx		; 함수에서 임시로 사용하는 레지스터를 스택에 저장, 함수의 마지막 부분에서 스택에 삽입된  값을 꺼내 복원
...
kReadTSC:
	...
	shl rdx, 32		; RDX 레지스터에 있는 상위 32비트 TSC값과 RAX 레지스터에 있는
...
kTestAndSet:
    ...
    ; 비교하여 두 값이 같다면 세 번째 파라미터의 값을 첫 번째 파라미터가 가리키는 어드레스에 삽입
